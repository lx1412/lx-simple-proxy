...
proxy.on('connect', (req, cltSocket, head) => {
    ...
    const onSrvSocketEnd = () => {
        cltSocket.end();
        srvSocket.removeListener('end', onSrvSocketEnd);
...
function auth(duplex, password) {
    ...
    let p = new Promise((resolve, reject) => {
        let timer = setTimeout(() => {
            duplex.emit('end', new Error('auth timeout!'));
        }, 10000);
        ...
        const ondata = chunk => {
            ...
            if (!!~index) {
                ...
                if (result === password) {
                    ...
                    duplex.emit('end', new Error('invalid password!'));
                }
            }
        };
...
const closeOnTimeout = (socket, ms, cb) => {
    ...
    socket.once('timeout', () => socket.end());
}
